...
rule FASTQC_raw:
	input:
		expand([os.path.join(config["input_DIR"], "{sample}_R1.fastq.gz")], sample = SAMPLES),
	output:
		html = expand([os.path.join(config["output_DIR"],"EVEREST/raw_fastqc/{sample}_R1_fastqc.html")], sample = SAMPLES),
	params:
		threads = "7",
...
rule multiQC_raw:
	input:
		expand([os.path.join(config["output_DIR"],"EVEREST/raw_fastqc/{sample}_R1_fastqc.html")], sample = SAMPLES),
	output:
		os.path.join(config["output_DIR"],"EVEREST/multiQC_rep/raw_qc_multiqc_report.html"),
	params:
		in_dir = os.path.join(config["output_DIR"],"EVEREST/raw_fastqc"),
