...
rule SAMTOOLS_fastq:
	...
	message:
		"obtain unmapped from star and convert to fastq",
	shell:
...
rule PIGZ_fastq:
	...
	output:
		expand([os.path.join(config["output_DIR"], "EVEREST/FASTQ/{sample}_unmapped_cat_R1.fastq.gz"), os.path.join(config["output_DIR"], "EVEREST/FASTQ/{sample}_unmapped_cat_R2.fastq.gz")], sample=SAMPLES),
	params:
		"-p 7 -5",
	log:
